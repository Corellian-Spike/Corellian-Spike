...
export type TGamePhase = 'Setup' | 'Ante' | 'Shuffle' | 'Deal' | 'Gain' | 'Spike' | 'Wager' | 'Score'
...
export class Game {
  ...
  // matchCurrentWagerByPlayer(player) {
  //   const difference = this.state.currentWager - player.currentWager;
  //   this.payByPlayerPotAndCredits(player, this.table.pot, difference);
  //   player.currentWager = this.state.currentWager;
  // };

  // matchAndRaiseCurrentWagerByPlayerAndCredits(player, credits) {
  //   this.matchCurrentWagerByPlayer(player);
  //   this.payByPlayerPotAndCredits(player, this.table.pot, credits);
  //   player.currentWager = player.currentWager + credits;
  //   this.state.currentWager = player.currentWager;
  // };
  ...
  initializePlayers(playerCount: number) {
    ...
    for (let playerNumber = 1; playerNumber <= playerCount; playerNumber++) {
      const player = new Player(`Player ${playerNumber}`, 250, this);
      players.push(player);
    };
    return players;
  };
  ...
  continue() {
    ...
    if (this.state.phase === 'Ante') {
      ...
      if (this.players.findIndex(player => player.id === nextActivePlayerId) < this.players.findIndex(player => player.id === this.state.turn)) {
        if (this.players.find(player => player.id === nextActivePlayerId)!.currentWager === this.state.currentWager) {
